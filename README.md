...
&nbsp;
2. For the submit feature, I created a submit page that users can get to by clicking the submit text on the homepage. This submit page has a form that takes their input, once they click submit, a POST request is sent to the /submit endpoint which I have set up in my server. In that endpoint is where the users word is checked to make sure it is of length 5, and then validated using the same dictionary API I used in my client-side. If it is of length 5 and is a valid word, it is accepted and the user is sent to a success page, however, it is also checked against my current database to make sure it is not already there, if it is, it won't be added as the word the user input is already in the database. If the users word is not of length 5 or not valid, the user is sent to an error page.
